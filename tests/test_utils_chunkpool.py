...
from simplezarr.utils.chunkpool import ChunkPool, ChunkLocation, create_chunk_pools_from_zarr_node
...
def test_chunk_pool_handlers():
    ...
    # Request some chunks
    chunk1a = pool.get_chunk(0, (0, 0), "r1a", load_handler=on_load1, drop_handler=on_drop, destroy_handler=on_destroy)
    chunk1b = pool.get_chunk(0, (0, 0), "r1b", load_handler=on_load1, drop_handler=on_drop, destroy_handler=on_destroy)

    chunk2 = pool.get_chunk(0, (0, 1), "r2", load_handler=on_load1, drop_handler=on_drop, destroy_handler=on_destroy)
    pool.get_chunk(0, (0, 1), "r2", load_handler=on_load2)

    # Load them
    pool.wait_for_chunks_to_load()

    assert events == ["load (0, 0) 1", "load (0, 0) 1", "load (0, 1) 1", "load (0, 1) 2"]
    events.clear()
...
def test_chunk_pool_handlers_async_load():
    ...
    chunk = pool.get_chunk(0, (0, 0), "r1", load_handler=on_load)
    assert events == []
